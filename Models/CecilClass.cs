  ...
  public class CecilClass : BaseCodeConstruct
  {
    ...
    public CecilClass(AssemblyReader assemblyReader, TypeDefinition cClass)
    {
      ...
      foreach (FieldDefinition f in cClass.Fields.Where(p1 => !p1.Name.Contains("__")))
      {
        ...
        fm.IsPrivate = f.IsPrivate;
        fm.IsStatic = f.IsStatic;
      ...
      foreach (PropertyDefinition p in cClass.Properties.Where(p1 => !p1.Name.Contains("__")))
      {
        if (p.GetMethod != null)
        {
          ...
          pm.IsPrivate = p.GetMethod.IsPrivate;
          pm.IsStatic = p.GetMethod.IsStatic;
          pm.IsAbstract = p.GetMethod.IsAbstract;
          pm.IsGetter = true;
        ...
        if (p.SetMethod != null)
        {
          ...
          pm.Name = p.Name;
          pm.FullName = p.FullName;
          pm.IsPrivate = p.SetMethod.IsPrivate;
          pm.IsStatic = p.SetMethod.IsStatic;
          pm.IsAbstract = p.SetMethod.IsAbstract;
          pm.IsSetter = true;
    ...
    private BaseCodeConstruct GetMethod(MethodDefinition method, string methodName, AssemblyReader assemblyReader)
    {
      if (method == null) return null;
      var pm = new BaseCodeConstruct();
      pm.Name = methodName;
      pm.IsPrivate = method.IsPrivate;
      pm.IsAbstract = method.IsAbstract;
      pm.IsStatic = method.IsStatic;
      CodeBlock cb = assemblyReader.GetMethod(method);
      if (cb.Body != "")
      {
        pm.CodeBlocks.Add(cb);
      }
      pm.Declaration = pm.GetDeclaration();
      pm = Common.ParseDeprecation(method, pm);
      return pm;
    }
    ...
    public override void WriteToDoc(ref XmlNode parent)
    {
      ...
      foreach (var bcc in Properties.OrderBy(t => t.Name))
      {
      ...
      foreach (var bcc in Events.OrderBy(t => t.Name))
      {
